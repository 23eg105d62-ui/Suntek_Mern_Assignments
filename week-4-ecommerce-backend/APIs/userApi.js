...
userRoute.post('/users', async (req, res) => {
    ...
    //res
    res.status(201).json({ message: "New user created", user: newUserDoc })
...
/*Add "quantity" filed  along with productId in user cart. 
...
If it is there , increment count by 1. 
If product is not there, then add new product to cart with quantity as 1
*/
...
userRoute.put("/user-cart/user-id/:uid/product-id/:pid", async (req, res) => {
    ...
    // Convert productId string to ObjectId (like compare route)
    ...
    // Flag to check product existence in cart
    let flag = false;

    // Compare ObjectIds 
    user.cart.forEach(item => {
        if (item.product.equals(productId)) {
            item.quantity += 1;     // If product already in cart increment quantity
            flag = true;
        }
    });

    // If product not found, push new product
    if (!flag) {
        user.cart.push({
            product: productId,
            quantity: 1
    ...
    await user.populate("cart.product", "prodName price");
    //Send Response
    res.status(200).json({
